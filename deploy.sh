...
docker build -t stpiep/multi-server:latest -t stpiep/multi-server:$SHA -f ./server/Dockerfile ./server
docker build -t stpiep/multi-worker:latest -t stpiep/multi-worker:$SHA -f ./worker/Dockerfile ./worker
